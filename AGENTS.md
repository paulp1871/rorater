...
- `src/App.tsx` is the root component: it gates on auth and wires up the routes.
- `src/pages/` holds route-level components (`HomePage`, `SearchPage`, `LoginScreen`), each with its own co-located `.css`.
- `src/components/` holds shared components such as `AppLayout` (the authenticated header/nav shell), each with its own co-located `.css`.
- `src/api/` owns `fetch` calls to the backend and the response types: `types.ts` (response shapes), `auth.ts` (session/OAuth), `users.ts` (search). All network access lives here, not in components. Import directly from the relevant module, e.g. `'../api/users'` or `'../api/types'`.
- `src/styles/` holds global styles: `index.css` (CSS variables — colors, fonts, `--accent`, etc.) and `base.css` (shared app-shell layout). Reuse those variables instead of hardcoding values.
...
The search endpoint is `GET /api/roblox/users/search?keyword=<string>`, returning `{ users: [...] }`. Keep the types in `src/api/types.ts` in sync with that response shape.
